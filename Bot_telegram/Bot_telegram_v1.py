...
class Bot_Telegram_trading():
    ...
    def Send_message(self, text):
        self._updater.dispatcher.bot.send_message("1686353548",text)


mybot = Bot_Telegram_trading(config.telegramBot_Token)
...
def Info_trading(update: Update, context: CallbackContext):
    ...
    string_text= string_text +  "Price " + str(TRADE_SYMBOL) + " now is: " + str(pricenow) + " USD \n"
...
def start_bot_telegram():
        mybot.start_bot()
        mybot._updater.dispatcher.add_handler(CommandHandler('start',start))
        mybot._updater.dispatcher.add_handler(CommandHandler('help', help))
        mybot._updater.dispatcher.add_handler(CommandHandler('balance', balance))
        mybot._updater.dispatcher.add_handler(CommandHandler('TotalProfit', TotalProfit))
        mybot._updater.dispatcher.add_handler(CommandHandler('Info_trading', Info_trading))
        #updater.dispatcher.add_handler(CommandHandler('Delete_Trade_Done', Firebase.Auto_delete_TradeDone()))

        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text('start'),start))
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text('help'),help))
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text('balance'),balance))
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text('profit'),TotalProfit))
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text('info trading'),Info_trading))
        #updater.dispatcher.add_handler(MessageHandler(Filters.text('Update_Delete_trading'),Firebase.Auto_delete_TradeDone()))
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text, unknown))


        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.command,unknown))  # Filters out unknown commands
        
        # Filters out unknown messages.
        mybot._updater.dispatcher.add_handler(MessageHandler(Filters.text, unknown_text))
        #bot.bots[token] = self
...
def bot_telegram_send(text):
    updater.dispatcher.bot.send_message("1686353548",text)
    """""" mybot= Bot_Telegram_trading("5020937139:AAGfNblKv-ohgSZCabNrLmNNvopZ_Bpl7qA")
    mybot.addHandler()
    mybot.start_bot()
    mybot.Send_message("hi, tôi đang test") """"""
